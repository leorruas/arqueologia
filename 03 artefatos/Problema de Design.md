---
title: "Problema de Design"
type: "artefato"
tags:
  - design/artefato
  - arqueologia
---

# Problema de Design

O Problema de Design é um artefato de formulação que enquadra e reconfigura as dores dos usuários, evitando que designers pulem direto para a implementação de soluções equivocadas.

## Ficha técnica
- **Criador(es)**: [[autores/Horst Rittel|Horst Rittel]], [[autores/Herbert Simon|Herbert Simon]]
- **Empresa/Fabricante**: Não aplicável
- **Ano de Lançamento**: Segunda metade do século XX.
- **Materiais/Mídia**: Briefs de design, quadros de problemas, dinâmicas de Double Diamond
- **Tipo de Design**: [[00 tipos de design/Design de Servicos|Design de Serviços]] / [[00 tipos de design/Design de Interface|Design de Interface]]

## História e contexto de criação
- **Demanda / Problema**: Como evitar resolver perfeitamente o problema errado.
- **Processo de Desenvolvimento**: Desenvolvido como ferramenta nos primeiros contextos de Segunda metade do século XX..

## Inovação e impacto
- **Ampliação de Capacidade**: Amplia o espaço de ideação criativa e a assertividade da solução desenhada.
- **Enfraquecimento de Capacidade**: Enfraquece a pressa para implementar soluções precoces baseadas apenas em suposições.
- **Comportamento Coletivo**: Não especificado
- **Linhagem e Descendência**: [[03 artefatos/Jobs to Be Done|Jobs to Be Done]], [[03 artefatos/Hipótese Científica|Hipótese Científica]], [[03 artefatos/pesquisa|pesquisa]], [[03 artefatos/design thinking|design thinking]]
- **Custo Invisível**: Esforço de tempo exigido para realizar pesquisas de enquadramento antes da codificação/produção.
- **O que ele simplifica**: A delimitação e o foco estratégico de workshops e processos criativos.
- **O que ele esconde**: As limitações tecnológicas e de orçamento durante a fase inicial de exploração.
- **Quem ele fortalece**: Designers estratégicos e de experiência do usuário.
- **Quem ele enfraquece**: Desenvolvedores e gerentes apressados pela entrega de telas rápidas.

## Referências e onde encontrar
- **Acervos**: Vitra Design Museum, MoMA (ou não aplicável se digital/processual)
- **Leituras**:
- [[00 índices/Livros Indicados|The Sciences of the Artificial]] por Herbert Simon
- *Dilemmas in a General Theory of Planning* por Horst Rittel e Melvin Webber

---

### Ficha resumo

- **Nome do artefato**: Problema de Design
- **Categoria**: Metodologia / Formulação / IHC
- **Período aproximado**: Segunda metade do século XX.
- **Pessoas ou equipes envolvidas**: [[autores/Horst Rittel|Horst Rittel]], [[autores/Herbert Simon|Herbert Simon]]
- **Produto onde apareceu**: Não especificado
- **Problema original**: Como evitar resolver perfeitamente o problema errado.
- **Hipótese sobre a natureza humana**: Não especificado
- **Relação de poder**: Não especificado
- **Mundo antes da inovação**: Construção de produtos baseados em briefs fechados focados exclusivamente na solução ditada pelo cliente.
- **A ideia que mudou tudo**: Reformular a demanda do cliente focando no enquadramento da real dor humana por trás dela.
- **Comportamento aproveitado**: Não especificado
- **Comportamento criado**: Não especificado
- **Por que funcionou**: Aproveitou a capacidade reflexiva dos designers para contornar preconcepções sobre o produto final.
- **Consequências inesperadas**: Paralisia por análise ('analysis paralysis') causada pela redefinição eterna do problema.
- **Legado**: Não especificado
- **Conceitos relacionados**: Não especificado
- **Família de ideias**: Artefatos de Formulação, Artefatos de Investigação.
- **Parentes**: [[03 artefatos/Jobs to Be Done|Jobs to Be Done]], [[03 artefatos/Hipótese Científica|Hipótese Científica]], [[03 artefatos/pesquisa|pesquisa]], [[03 artefatos/design thinking|design thinking]]
- **Hipótese central**: A eficácia do design reside mais em descobrir a pergunta certa do que em refinar respostas apressadas.
- **Princípio de design revelado**: Formular corretamente o problema é metade da solução.
- **Destino do artefato**: Metodologia central das disciplinas de IHC, Design de Serviços e Design Thinking.
- **Perguntas em aberto**: Como definir o momento ideal de parar de reformular e partir para a construção de protótipos?
