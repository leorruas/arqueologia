---
title: "Portfólio"
type: "artefato"
tags:
  - design/artefato
  - arqueologia
---

# Portfólio

O Portfólio é um artefato de evidência que torna observáveis capacidades e evoluções profissionais por meio de produtos reais de trabalho.

## Ficha Técnica
- **Criador(es)**: Não aplicável
- **Empresa/Fabricante**: Não aplicável
- **Ano de Lançamento**: Século XIX entre artistas e arquitetos; amplamente difundido no século XX.
- **Materiais/Mídia**: Papel impresso, encadernações, websites, plataformas (Behance, GitHub)
- **Tipo de Design**: [[00 tipos de design/Design Grafico|Design Gráfico]] / [[00 tipos de design/Design de Interface|Design de Interface]]

## História e Contexto de Criação
- **Demanda / Problema**: Como tornar competências invisíveis observáveis.
- **Processo de Desenvolvimento**: Desenvolvido como ferramenta nos primeiros contextos de Século XIX entre artistas e arquitetos; amplamente difundido no século XX..

## Inovação e Impacto
- **Ampliação de Capacidade**: Amplia a capacidade de demonstrar competência prática e autoeficácia.
- **Enfraquecimento de Capacidade**: Enfraquece a necessidade de avaliações puramente teóricas ou credenciamentos burocráticos.
- **Comportamento Coletivo**: Documentar evolução.
- **Linhagem e Descendência**: [[03 artefatos/GitHub|GitHub]], [[03 artefatos/currículo|currículo]], [[03 artefatos/diário de projeto|diário de projeto]], [[03 artefatos/histórico de versões|histórico de versões]]
- **Custo Invisível**: Esforço constante de curadoria e autopromoção permanente.
- **O que ele simplifica**: A validação rápida de capacidades e estilo de trabalho.
- **O que ele esconde**: As falhas, retrabalhos e a ajuda de terceiros nos projetos selecionados.
- **Quem ele fortalece**: Designers e criativos autônomos buscando validação no mercado.
- **Quem ele enfraquece**: Modelos tradicionais de contratação focados exclusivamente em diplomas.

## Referências e Onde Encontrar
- **Acervos**: Vitra Design Museum, MoMA (ou não aplicável se digital/processual)
- **Leituras**:
- *Show Your Work!* por Austin Kleon

---

### Ficha Resumo

- **Nome do artefato**: Portfólio
- **Categoria**: Evidência / Avaliação / Identidade
- **Período aproximado**: Século XIX entre artistas e arquitetos; amplamente difundido no século XX.
- **Pessoas ou equipes envolvidas**: Não aplicável
- **Produto onde apareceu**: Não especificado
- **Problema original**: Como tornar competências invisíveis observáveis.
- **Hipótese sobre a natureza humana**: As pessoas confiam mais em evidências do que em declarações.
- **Relação de poder**: Pode favorecer reconhecimento ou transformar toda produção em vitrine permanente.
- **Mundo antes da inovação**: Contratação baseada unicamente em currículos descritivos ou recomendações pessoais.
- **A ideia que mudou tudo**: Apresentar evidências concretas do resultado do próprio trabalho em vez de autodeclarações.
- **Comportamento aproveitado**: Não especificado
- **Comportamento criado**: Documentar evolução.
- **Por que funcionou**: Aproveitou a preferência humana por validações empíricas e visuais.
- **Consequências inesperadas**: Cultura de hiper-estetização que prioriza o apelo visual do portfólio sobre a funcionalidade real do objeto.
- **Legado**: Não especificado
- **Conceitos relacionados**: Não especificado
- **Família de ideias**: Artefatos de Evidência, Artefatos de Competência.
- **Parentes**: [[03 artefatos/GitHub|GitHub]], [[03 artefatos/currículo|currículo]], [[03 artefatos/diário de projeto|diário de projeto]], [[03 artefatos/histórico de versões|histórico de versões]]
- **Hipótese central**: A evidência de realizações passadas é o indicador mais confiável de competências futuras.
- **Princípio de design revelado**: Mostrar resultados convence mais do que descrever capacidades.
- **Destino do artefato**: Migrou inteiramente para plataformas globais online de design e repositórios de código.
- **Perguntas em aberto**: Como avaliar processos qualitativos invisíveis apenas observando portfólios finais?
