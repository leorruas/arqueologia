---
title: "Princípio"
type: "artefato"
tags:
  - design/artefato
  - arqueologia
---

# Princípio

O Princípio é uma heurística organizacional estruturada para orientar o julgamento subjetivo em situações inéditas onde regras explícitas são insuficientes.

## Ficha Técnica
- **Criador(es)**: Não aplicável
- **Empresa/Fabricante**: Não aplicável
- **Ano de Lançamento**: Muito anterior às organizações modernas. Ganha força em empresas a partir da segunda metade do século XX.
- **Materiais/Mídia**: Documentos de diretrizes, cartazes, códigos de ética
- **Tipo de Design**: [[00 tipos de design/Design de Servicos|Design de Serviços]]

## História e Contexto de Criação
- **Demanda / Problema**: Como orientar decisões quando não existe uma regra específica.
- **Processo de Desenvolvimento**: Desenvolvido como ferramenta nos primeiros contextos de Muito anterior às organizações modernas. Ganha força em empresas a partir da segunda metade do século XX..

## Inovação e Impacto
- **Ampliação de Capacidade**: Amplia a autonomia de decisão e julgamento individual em contextos dinâmicos.
- **Enfraquecimento de Capacidade**: Enfraquece a dependência de regulamentações exaustivas e POPs rígidos.
- **Comportamento Coletivo**: Decisão contextual em vez de execução mecânica.
- **Linhagem e Descendência**: [[03 artefatos/Valores|Valores]], [[03 artefatos/credo organizacional|credo organizacional]], [[03 artefatos/constituição|constituição]], [[03 artefatos/Manifesto|Manifesto]]
- **Custo Invisível**: Inconsistência potencial nas decisões tomadas por pessoas diferentes sob o mesmo princípio.
- **O que ele simplifica**: A tomada de decisões complexas e adaptativas.
- **O que ele esconde**: As ambiguidades e os trade-offs de decisões difíceis.
- **Quem ele fortalece**: Profissionais e equipes operacionais de primeira linha.
- **Quem ele enfraquece**: Burocratas que dependem de regras estritas.

## Referências e Onde Encontrar
- **Acervos**: Vitra Design Museum, MoMA (ou não aplicável se digital/processual)
- **Leituras**:
- [[00 índices/Livros Indicados|The Design of Everyday Things]] por Donald Norman (sobre princípios aplicados a interfaces)
- *Customer Obsession* (Amazon Principles)

---

### Ficha Resumo

- **Nome do artefato**: Princípio
- **Categoria**: Gestão / Decisão / Estratégia
- **Período aproximado**: Muito anterior às organizações modernas. Ganha força em empresas a partir da segunda metade do século XX.
- **Pessoas ou equipes envolvidas**: Não aplicável
- **Produto onde apareceu**: Não especificado
- **Problema original**: Como orientar decisões quando não existe uma regra específica.
- **Hipótese sobre a natureza humana**: As pessoas conseguem interpretar situações novas quando possuem critérios claros.
- **Relação de poder**: Pode distribuir autonomia ou servir como justificativa vaga para decisões arbitrárias.
- **Mundo antes da inovação**: Paralisia operacional ou paralisia decisória diante de imprevistos não regulamentados.
- **A ideia que mudou tudo**: Fornecer critérios de decisão adaptáveis em vez de procedimentos.
- **Comportamento aproveitado**: Não especificado
- **Comportamento criado**: Decisão contextual em vez de execução mecânica.
- **Por que funcionou**: Aproveitou a capacidade de julgamento das pessoas sob critérios coerentes.
- **Consequências inesperadas**: Uso de princípios como justificativas vagas para decisões de poder arbitrárias.
- **Legado**: Não especificado
- **Conceitos relacionados**: Não especificado
- **Família de ideias**: Artefatos de Autodeterminação, Artefatos de Escolha.
- **Parentes**: [[03 artefatos/Valores|Valores]], [[03 artefatos/credo organizacional|credo organizacional]], [[03 artefatos/constituição|constituição]], [[03 artefatos/Manifesto|Manifesto]]
- **Hipótese central**: Bons critérios gerais produzem melhores soluções locais do que procedimentos inflexíveis.
- **Princípio de design revelado**: Julgamento local supera regulamentações globais excessivas.
- **Destino do artefato**: Evoluiu para princípios de design digital, frameworks ágeis e filosofias corporativas.
- **Perguntas em aberto**: Como medir o alinhamento real de uma equipe em relação a um princípio abstrato?
