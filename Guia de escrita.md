...
A abertura deve começar por um problema, uma cena, um conflito ou uma pergunta concreta. Evitar começar com ficha técnica ou definição enciclopédica quando houver uma entrada narrativa mais útil.
...
Quando houver disputa de autoria, apresentar as versões relevantes e explicar por que a disputa existe.
...
Não usar uma obra teórica como prova de um fato histórico apenas porque ela é conceitualmente relacionada ao tema.

Até que a camada pública de referências seja implantada de forma consistente, manter as fontes no Markdown e revisar sua qualidade progressivamente.

## Relações entre artefatos

Ao dizer que dois artefatos são parentes, indicar a natureza da relação quando ela não for evidente: histórica, funcional, semiótica, comportamental ou hipótese comparativa.
...
Cada seção deve acrescentar uma nova camada da investigação.
...
- presença de fontes adequadas para afirmações históricas centrais;
- ficha arqueológica ao final do artefato;
- leitura fluida antes da camada documental.
