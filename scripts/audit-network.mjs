...
const porTituloNaCategoria = new Map();
for (const a of arquivos.filter(x => x.path !== "index.md" && !x.path.startsWith("00 índices/"))) {
  const categoria = a.path.split("/")[0];
  const chave = `${categoria}::${normalizar(a.title)}`;
  if (!porTituloNaCategoria.has(chave)) porTituloNaCategoria.set(chave, []);
  porTituloNaCategoria.get(chave).push(a.path);
}
for (const paths of porTituloNaCategoria.values()) {
  if (paths.length > 1) {
    for (const p of paths) add("duplicate-title", p, `Título normalizado duplicado na mesma camada entre: ${paths.join(", ")}.`);
  }
}
...
const relatorio = {
  ...
  scope: "Integração do grafo: títulos duplicados dentro da mesma camada, índices, home, reciprocidade estrutural, tipos de design e nós órfãos. Conceitos e variáveis não exigem backlink exaustivo.",
